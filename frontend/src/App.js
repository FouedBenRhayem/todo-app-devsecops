...
export default function App() {
  ...
  const deleteTask = useCallback((id) => {
    ...
    fetch(`${API_URL}/tasks/${id}`, { method: "DELETE" }).catch((err) =>
        ...
        <div className="wrap">
          ...
          <div className="main">
            <form className="form-panel liquid-glass-strong" onSubmit={addTask}>
                ...
                <div className="prio-row">
                  {["High", "Medium", "Low"].map((p) => (
                    <button
                      key={p}
                      type="button"
                      className={`pill-btn ${form.priority === p ? "active" : ""}`}
              ...
              <button className="submit-btn hoverable" type="submit">
              ...
              <div className="task-list">
                {filtered.map((task) => {
                  ...
                  const badgeBg = isDark
                    ? `rgba(255,255,255,${0.08 + op * 0.14})`
                    : `rgba(0,0,0,${0.05 + op * 0.12})`;
                  return (
                    <div key={task.id} className="task-row liquid-glass row-hoverable">
                      <button
                        className={`check-btn hoverable ${task.done ? "done" : ""}`}
                      ...
                      <div className="task-body">
                        <div className="task-head">
                          <span className="task-title" style={{ textDecoration: task.done ? "line-through" : "none" }}>
                            {task.title}
                          </span>
